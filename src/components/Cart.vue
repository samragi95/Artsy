<template>
   ...
   <p class="text-center py-20" v-show="!products.length">
    	<i class="font-semibold text-3xl not-italic ">Your cart is empty!!</i><br>

    	<router-link class="bg-orange-100 outline px-4 relative top-8 py-4" to="/Home">Explore More!!</router-link>
   </p>
    	...
    	<tbody>
    		<tr v-for="(p,id) in products" :key="id">
        		...
        		<td class=" py-2  px-6 whitespace-nowrap bg-slate-200">{{ p.quantity }}</td>
        	...
        	<tr>
              ...
              <td class="px-6">
        		...
        		<td></td>
        		<td class=" relative left-5 "><b>${{ total }}</b></td>
        	</tr>
    	</tbody>
    ...
    <p class="text-center"><button v-show="products.length" class="bg-slate-400 px-4 rounded-md outline-hidden hover:outline " v-on:click='checkout'>Checkout</button></p>
    <Footer />  
  </div>
